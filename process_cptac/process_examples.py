...
phospho_df_HSNCC.index = phospho_df_HSNCC.index.map(lambda g: lookup(g.split('.')[0]) if lookup(g.split('.')[0]) else g.split('.')[0])
phospho_df_HSNCC.to_csv('out/HSNCC_phospho_observed.tsv', sep='\t')

# %%
prot_df = pd.read_csv('data/HNSCC_proteomics_gene_abundance_log2_reference_intensity_normalized_Tumor.txt', sep='\t', index_col=0)
prot_df.index = prot_df.index.map(lambda g: lookup(g.split('.')[0]) if lookup(g.split('.')[0]) else g.split('.')[0])
prot_df.to_csv('out/HNSCC_proteomics_tumor.tsv', sep='\t')
# %%
